...
/*
    ...
    This is a modified version of v1.07 that adds an empty title to the header if none is provided.
    It also allows custom padding size
*/
...
//---------------------------------------------------------------------------------
{
        ...
        if (ARGV[0] == '-')
        {
            switch (ARGV[1])
            {
                ...
                case 't':    // title
                {
                    ...
                    {
                        // use good header's empty title
                        memcpy(header.title, good_header.title, sizeof(header.title));
                        break;
                    }
    ...
    #define CHECK (power != 0 && power >= bit ? power : bit)
    if (schedule_pad) {
        if (sh_offset != 0) {
